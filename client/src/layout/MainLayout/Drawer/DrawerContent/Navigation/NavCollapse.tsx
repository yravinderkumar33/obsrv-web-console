...
const NavCollapse = ({ menu, level, handleDrawerToggle }: Props) => {
    ...
    const navCollapse = menu.children?.map((item) => {
        switch (item.type) {
            case 'collapse':
                return <NavCollapse key={item.id} menu={item} level={level + 1} handleDrawerToggle={handleDrawerToggle} />;
            case 'item':
                return <NavItem key={item.id} item={item} level={level + 1} handleDrawerToggle={handleDrawerToggle} />;
            default:
                return (
