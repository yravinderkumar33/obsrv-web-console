...
const getMasterDatasets = (datasets: Array<any>) => {
    return _.filter(datasets, (dataset: Record<string, any>) => _.get(dataset, 'type') === "master-dataset" && ['ACTIVE', 'PUBLISHED', 'READY_FOR_PUBLISH',].includes(_.get(dataset, 'status')));
}

const getRedisConfig = (datasets: Array<any>) => {
    const data = _.filter(datasets, dataset => _.get(dataset, 'type') === "master-dataset" && ['ACTIVE', 'PUBLISHED', 'READY_FOR_PUBLISH',].includes(_.get(dataset, 'status')));
    if (data.length > 0)
        return {
